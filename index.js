...
app.get("/get-weather", async (req, res) => {
  try {
    ...
    if (!city || !state || !country) {
      throw new Error(
        " Please fill in all the fields using the proper syntax as shown in the examples before searching."
      );
    }

    const response = await axios.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city},${state},${country}&limit=1&units=imperial&appid=${API_KEY}`
    );
    ...
    if (error.response && error.response.status === 404) {
      ...
      res.render("weather.ejs", {
        error:
          "ERROR: City not found. Please provide a valid city, state, and country.",
      });
    } else {
      // Handle other errors
